...
const googlePostBuild = async (reporter: Reporter) => {
  ...
  if (!client_email || !private_key) {
    reporter.info("Google Indexing skipped - GOOGLE_CLIENT_EMAIL and GOOGLE_PRIVATE_KEY environment variables not set");
    return;
  }

  // Variables
  const formatted_private_key = private_key.split(String.raw`\n`).join("\n");
